...
const Employee = ({
  ...
  const deleteShift = (shiftId) => {
    debugger;
    const newEmpShifts = employee.shifts.filter(
      ...
      <div>
        <h2>
          ...
          <span
            ...
            onClick={() => setEmployeeSelected(false)}
          >
            X
          </span>
        </h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <strong>
            Add Shifts:{" "}
            <select name="shifts" onChange={handleShiftsAddition} multiple>
              <option value="" disabled>
                Add shifts
              </option>

              {allShifts?.map((shift, index) => {
                const exist = employee.shifts?.find(
                  (empShift) => empShift._id == shift._id
                );
                if (!exist) {
                  return (
                    <option key={index} value={shift._id}>
                      {" "}
                      {`${shift.date} ${shift.startingHr} ${shift.endingHr}`}
                    </option>
                  );
                }
              })}
